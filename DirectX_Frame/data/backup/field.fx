	...
	out float4 out_color : COLOR0)			//ピクセルカラー出力
{
	...
	if (g_tex) 
	{
		float4 base_diffuse = 1.0f;
		float4 rock_color = tex2D(RockSampler1, in_tex1);
		float4 grass_color = tex2D(GrassSampler1, in_tex1);
		float4 soil_color = tex2D(SoilSampler1, in_tex1);
		rock_color = diffuse * rock_color * in_Diff.x;
		//rock_color.w = in_Diff.x/* / (in_Diff.x + in_Diff.y + in_Diff.z)*/;
		grass_color = diffuse * grass_color * in_Diff.y;
		//grass_color.w = in_Diff.y / (in_Diff.x + in_Diff.y + in_Diff.z);
		//soil_color = diffuse * soil_color;
		//soil_color.w = in_Diff.z / (in_Diff.x + in_Diff.y + in_Diff.z);

		// テクスチャの色と頂点の色を掛け合わせる
		float4 mix_color = rock_color + grass_color/* + soil_color*/;
		mix_color.w = 1.0f;
		out_color = saturate(mix_color);
	}
	else 
	{
