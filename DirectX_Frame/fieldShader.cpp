...
CShaderField *CShaderField::m_Shader = NULL;

CShaderField::CShaderField() : CShader()
{
	bool sts;
	sts = VertexShaderCompile(
		SHADER_FILE[SH_ID_FIELD].c_str(),						// シェーダーファイル名
		"main",							// エントリー関数名
		"vs_3_0");						// バージョン
...
CShaderField* CShaderField::GetShader()
{
	if (m_Shader == NULL)
	{
		m_Shader = new CShaderField();
	}

	return m_Shader;
}

void CShaderField::Destroy()
{
	if (m_Shader)
	{
		delete m_Shader;
...
void CShaderField::ShaderSet(const D3DXMATRIX& world)
{
	...
	m_VSConstantTable->SetMatrix(pDevice, "g_light_view", &CManager::GetLightCamera());
	m_VSConstantTable->SetMatrix(pDevice, "g_light_projection", &CManager::GetLightProjection());
	...
	m_VSConstantTable->SetMatrix(pDevice, "g_matuv", &matuv);

}

void CShaderField::SetMaterial(D3DMATERIAL9 const&mat)
{
	LPDIRECT3DDEVICE9 pDevice = CRenderer::GetDevice();
	if (pDevice == NULL)
	{
	...
	// 投影シャドウテクスチャをシェーダーへセット
	int index = m_PSConstantTable->GetSamplerIndex("ProjectionShadowSampler");
	pDevice->SetTexture(index, CManager::GetShadowTexture());
