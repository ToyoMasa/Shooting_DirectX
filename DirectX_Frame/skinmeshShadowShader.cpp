...
CShaderSkinmeshShadow *CShaderSkinmeshShadow::m_Shader = NULL;

CShaderSkinmeshShadow::CShaderSkinmeshShadow() : CShader()
{
	bool sts;
	sts = VertexShaderCompile(
		SHADER_FILE[SH_ID_SKINMESH_SHADOW].c_str(),						// シェーダーファイル名
		"main",							// エントリー関数名
		"vs_3_0");						// バージョン
...
CShaderSkinmeshShadow* CShaderSkinmeshShadow::GetShader()
{
	if (m_Shader == NULL)
	{
		m_Shader = new CShaderSkinmeshShadow();
	}

	return m_Shader;
}

void CShaderSkinmeshShadow::Destroy()
{
	if (m_Shader)
	{
		delete m_Shader;
...
void CShaderSkinmeshShadow::ShaderSet(const D3DXMATRIX& world)
{
...
void CShaderSkinmeshShadow::SetMaterial(D3DMATERIAL9 const&mat)
{
